...
const App: React.FC = () => {
        ...
        try {
            ...
            try {
                ...
                if (Array.isArray(plansRes.data)) {
                    setAppState(prev => ({ ...prev, subscriptionPlans: plansRes.data }));
                }
            } catch (e) { console.warn("Plans fetch failed", e); }
  ...
  const handleSaveSale = async (sale: SaleRecord) => withLoading(async () => {
      ...
      setAppState(prev => {
          ...
          if (existing) return { ...prev, sales: prev.sales.map(s => s.id === sale.id ? data : s) };
  ...
  const handleDeleteSale = async (id: string, reason: string) => withLoading(async () => {
      ...
      setAppState(prev => ({ 
          ...prev, sales: updatedSales.data, logs: updatedLogs.data, inventory: updatedInv.data
      }));
        ...
        <header className="shrink-0 z-30 relative mx-2 mt-2 md:mx-4 md:mt-4 rounded-xl md:rounded-2xl border border-white/60 shadow-sm glass-panel flex items-center justify-between px-3 md:px-6 py-2 md:py-0 h-16">
            <div className="flex items-center space-x-2 md:space-x-3">
                {currentView !== 'dashboard' && (
                    <button onClick={() => setCurrentView('dashboard')} className="p-2 rounded-xl bg-slate-800 text-white hover:scale-105 transition-transform shadow-md flex items-center space-x-2 px-3">
            ...
            <button onClick={() => setIsSpotlightOpen(true)} className="hidden md:flex items-center bg-slate-100/50 hover:bg-slate-100 border border-slate-200/50 rounded-xl px-4 py-2 text-sm text-slate-500 transition-all group">
            ...
            <button onClick={() => setIsSpotlightOpen(true)} className="md:hidden p-2 text-slate-600 bg-white/50 rounded-lg"><Search className="w-5 h-5" /></button>
