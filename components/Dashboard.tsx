...
const AppIcon: React.FC<{ icon: React.FC<any>; gradient: string; size?: 'sm' | 'md' | 'lg' }> = ({ icon: Icon, gradient, size = 'md' }) => {
  const sizeClasses = {
    ...
    md: 'w-10 h-10 p-2 rounded-2xl', // Reduced from w-14 for compact fit
    lg: 'w-12 h-12 p-3 rounded-2xl'
  };
  
  return (
    <div className={`${sizeClasses[size]} ${gradient} shadow-lg shadow-black/5 ring-2 ring-white/40 flex items-center justify-center text-white transform transition-all duration-500 ease-out group-hover:scale-110 group-hover:rotate-3 group-hover:shadow-xl`}>
...
const HDStatWidget: React.FC<{ 
    ...
    return (
        <div className={`flex flex-col justify-between p-4 rounded-3xl border ${colors.border} ${colors.bg} backdrop-blur-md shadow-sm hover:shadow-md transition-all duration-300 hover:-translate-y-1 ${colors.hover} group cursor-default`}>
            <div className="flex justify-between items-start mb-2">
                <div className={`p-1.5 rounded-xl ${colors.icon} text-white shadow-md ring-2 ring-white/60 group-hover:scale-110 transition-transform`}>
                    <Icon className="w-4 h-4 stroke-[3px]" />
                </div>
                {trendPercent !== undefined && (
                    <span className={`flex items-center gap-0.5 text-[10px] font-bold px-2 py-1 rounded-full border bg-white/90 shadow-sm ${finalIsNegative ? 'text-rose-600 border-rose-100' : 'text-emerald-600 border-emerald-100'}`}>
            ...
            <div>
                ...
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mt-0.5 opacity-80">{label}</p>
            </div>
        </div>
    );
};

// COMPACT Navigation Module
    ...
    return (
        <button 
            ...
            className={`
                ...
                shadow-sm hover:shadow-xl hover:shadow-${colorTheme}-500/10 
                hover:-translate-y-1 hover:scale-[1.02] active:scale-[0.98]
                transition-all duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)]
                min-h-[110px] justify-center
            `}
        >
            {/* Notification Badge */}
            {count !== undefined && count > 0 && (
                <div className="absolute top-2 right-2 bg-rose-500 text-white text-[9px] font-black px-1.5 py-0.5 rounded-full shadow-lg shadow-rose-500/30 z-20 animate-pulse border-2 border-white ring-1 ring-rose-200">
                    {count}
                </div>
            )}
            ...
            <h3 className="text-sm font-black text-slate-800 group-hover:text-black leading-tight tracking-tight">{title}</h3>
            <p className="text-[10px] text-slate-500 font-bold mt-0.5 leading-tight opacity-70 uppercase tracking-wide group-hover:opacity-100 transition-opacity">{desc}</p>
        </button>
    );
};
...
const Dashboard: React.FC<DashboardProps> = ({ state, onNavigate, onSelectPatient }) => {
  ...
  return (
    // Reduced outer padding for tighter fit
    <div className="h-full overflow-y-auto custom-scrollbar bg-cyan-50/40 p-2 lg:p-3">
      
      <div className="max-w-[1900px] mx-auto bg-white/40 backdrop-blur-2xl rounded-[2.5rem] p-4 lg:p-6 border-[4px] border-white shadow-xl shadow-cyan-900/5">
        
        {/* Changed XL gap to be tighter */}
        <div className="flex flex-col xl:flex-row gap-4">
            
            {/* --- LEFT COLUMN --- */}
            {/* Reduced width from 300px to 240px for desktop */}
            <div className="xl:w-[240px] shrink-0 flex flex-col gap-4 animate-slide-up">
                
                {/* 1. Identity Card (Compact) */}
                <div className="bg-gradient-to-br from-cyan-50 via-white to-blue-50 p-4 rounded-[2rem] border border-white/60 shadow-lg shadow-cyan-900/5 hover:shadow-xl hover:border-cyan-200 transition-all duration-500">
                    <div className="flex flex-col items-center text-center gap-2">
                        <div className={`w-16 h-16 rounded-[1.5rem] shadow-lg flex items-center justify-center text-white text-2xl font-black ring-4 ring-white ${getAvatarGradient(state.currentUser?.name || '')}`}>
                            {state.currentUser?.avatarUrl ? (
                                <img src={state.currentUser.avatarUrl} className="w-full h-full object-cover rounded-[1.5rem]" />
                            ) : (
                                state.currentUser?.name.charAt(0)
                            )}
                        </div>
                        <div>
                            <h2 className="text-lg font-black text-slate-800 leading-tight tracking-tight">{state.currentUser?.name.split(' ')[0]}</h2>
                            <p className="text-[10px] font-bold text-cyan-600 uppercase tracking-widest mt-0.5">{currentTenant?.name}</p>
                ...
                <div className="flex-1 bg-gradient-to-br from-indigo-50/80 via-white to-violet-50/50 p-4 rounded-[2rem] border border-white/60 shadow-lg shadow-indigo-900/5 flex flex-col relative overflow-hidden min-h-[350px]">
                    <div className="flex justify-between items-end mb-4 z-10 relative">
                        <div>
                            ...
                            <p className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest">{new Date().toLocaleString('default', { month: 'short', weekday: 'short' })}</p>
                        </div>
                    ...
                    {/* Timeline Line */}
                    <div className="absolute left-[2.85rem] top-28 bottom-0 w-[2px] bg-indigo-100 z-0 dashed"></div>

                    <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar space-y-3 z-10">
                        {todayAppointments.length === 0 ? (
                            <div className="flex flex-col items-center justify-center h-40 opacity-50">
                                <Clock className="w-10 h-10 text-indigo-300 mb-2"/>
                            ...
                            todayAppointments.map((apt, idx) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {/* Timeline Dot */}
                                        <div className={`absolute left-[2.6rem] w-2.5 h-2.5 rounded-full border-[2px] z-20 transition-all ${isNext ? 'bg-indigo-500 border-indigo-200 shadow-[0_0_0_3px_rgba(99,102,241,0.2)]' : 'bg-white border-slate-300'}`}></div>
                                        
                                        {/* Appointment Card - Compact */}
                                        <div className={`flex-1 p-2.5 rounded-[1rem] border transition-all shadow-sm group-hover:shadow-md relative overflow-hidden ${isNext ? 'bg-white border-indigo-100 ring-2 ring-indigo-50' : 'bg-white/60 border-transparent hover:bg-white'}`}>
            ...
            <div className="flex-1 flex flex-col gap-4 animate-slide-up" style={{ animationDelay: '100ms' }}>
                ...
                <div className="relative overflow-hidden rounded-[2.5rem] bg-gradient-to-r from-teal-50 via-white to-rose-50 p-6 shadow-xl shadow-teal-900/5 ring-4 ring-white border border-teal-50/50 group">
                    <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
                        <div>
                            ...
                            <h1 className="text-3xl lg:text-4xl font-black mb-2 tracking-tight text-slate-800">{displayName}</h1>
                            <p className="text-slate-600 font-medium text-sm md:max-w-lg leading-snug">
                                You have <span className="font-bold text-teal-600 bg-teal-50 px-1.5 rounded-md border border-teal-100">{todayAppointments.length} appointments</span>. 
                                {pendingLabs > 0 && <span> <span className="font-bold text-rose-500 bg-rose-50 px-1.5 rounded-md border border-rose-100">{pendingLabs} lab results</span> awaiting.</span>}
                            </p>
                        </div>
                        <div className="flex gap-2">
                            <button 
                                onClick={() => onNavigate('logs')}
                                className="flex items-center gap-2 bg-slate-800 text-white px-5 py-3 rounded-2xl text-xs font-bold shadow-lg shadow-slate-800/20 hover:shadow-xl hover:scale-105 active:scale-95 transition-all group-hover:bg-black"
                            >
                                <FileClock className="w-3.5 h-3.5 stroke-white" />
                                <span>Logs</span>
                    ...
                    {/* Decorative Blobs */}
                    <div className="absolute top-0 right-0 w-60 h-60 bg-teal-300 opacity-10 rounded-full blur-[60px] translate-x-20 -translate-y-10 group-hover:opacity-20 transition-opacity duration-700"></div>
                    <div className="absolute bottom-0 right-20 w-40 h-40 bg-rose-300 opacity-10 rounded-full blur-[40px] translate-y-20 group-hover:opacity-20 transition-opacity duration-700"></div>
                </div>

                {/* 2. Application Grid (Compact) - Reduced gap from 5 to 3 */}
            ...
            {/* Reduced width from 300px to 240px */}
            <div className="xl:w-[240px] shrink-0 flex flex-col gap-4 animate-slide-up" style={{ animationDelay: '200ms' }}>
                ...
                {/* 2. Visual Chart Card - Reduced min height */}
                <div className="flex-1 bg-white p-4 rounded-[2rem] border border-slate-100 shadow-lg shadow-teal-900/5 flex flex-col min-h-[200px] relative overflow-hidden group hover:border-teal-100 transition-colors">
                    <div className="flex items-center justify-between mb-2 z-10">
                        <h3 className="text-sm font-black text-slate-700 tracking-tight">Finances</h3>
                        <div className="p-1.5 bg-teal-50 text-teal-600 rounded-lg">
                            <Activity className="w-3.5 h-3.5" />
                        </div>
                    </div>
                    
                    <div className="flex-1 -ml-5 absolute bottom-0 left-0 right-0 top-10 z-0 opacity-80 group-hover:opacity-100 transition-opacity">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={chartData}>
                                ...
                                <Tooltip 
                                    ...
                                    itemStyle={{ color: '#0f766e', fontWeight: 'bold', fontSize: '11px' }}
                ...
                {lowStockCount > 0 && (
                    <button 
                        onClick={() => onNavigate('inventory')}
                        className="p-3 bg-rose-50 border border-rose-100 rounded-[2rem] flex items-center gap-3 hover:bg-rose-100 hover:border-rose-200 transition-all text-left group shadow-sm hover:shadow-md hover:-translate-y-1"
                    >
                        <div className="w-10 h-10 shrink-0 rounded-2xl bg-rose-500 text-white flex items-center justify-center shadow-lg shadow-rose-500/30 group-hover:scale-110 group-hover:rotate-6 transition-all">
                        ...
                        <div>
                            ...
                            <p className="text-xs font-bold text-rose-900/80 leading-tight mt-0.5">{lowStockCount} low items</p>
                        </div>
                    </button>
                )}
            </div>
